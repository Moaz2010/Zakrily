...
function LegacyPracticePage() {
  ...
  useEffect(() => {
    ...
    practiceApi
      ...
      .catch((err) => {
        if (active) {
          setError(err.message ?? "تعذر تحميل التدريب");
          setLoading(false);
        }
      });
    return () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0E0E0E] text-white">
      ...
      <div style={{ backgroundColor: config.bg }} className="px-5 pt-12 pb-4">
        ...
        <h1 className="text-white font-black text-2xl text-end">تدريب ذكي: {config.ar}</h1>
        <p className="text-white/70 text-sm text-end mt-1">أسئلة مخصصة لتقوية مهاراتك</p>
      </div>
        ...
        {result && (
          <div className="rounded-2xl bg-[#1A1A1A] p-6 border border-white/5 flex flex-col gap-5 text-end">
            <div className="flex flex-col items-center justify-center my-2">
              ...
              <h2 className="text-2xl font-black text-white">أحسنت!</h2>
              <p className="text-white/70 text-sm mt-1">اكتملت جلسة التدريب الذكي بنجاح</p>
            </div>
          ...
          <div className="flex-1 flex flex-col justify-between">
            ...
            <div className="flex items-center gap-3 pt-6 pb-2">
              ...
              {isLast ? (
                ...
                >
                  {submitting ? "جاري الإرسال..." : "إنهاء التدريب"}
                </button>
              ) : (
                <button
                  onClick={() => setCurrentIndex((prev) => prev + 1)}
